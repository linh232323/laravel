...
<div class="panel-body">
		{!!Form::open(array('route'=>array("admin.transporter.update",$data->id),'method'=>'PUT','id'=>'form-attribute-group','class'=>'form-horizontal','files'=>true))!!} 
		    	...
		    	@foreach($languages as $language)
					@if($data->attentions()->where('languageID',$language->id)->first())
				    	...
				    	<div class="input-group"><span class="input-group-addon"><img src="{{URL::asset('public/admin/image/flags/'.$language->image)}}" title="{{$language->name}}"></span>
				      	...
				      	@else
				    	<div class="input-group"><span class="input-group-addon"><img src="{{URL::asset('public/admin/image/flags/'.$language->image)}}" title="{{$language->name}}"></span>
				        	<input type="text" name="transporter_{{$language->id}}" value="{{$data->descriptions()->where('languageID',$language->id)->first()->name}}" placeholder="Transporter Name" class="form-control">
				      	</div>
				      	@endif
			      	@else
				    	<div class="input-group"><span class="input-group-addon"><img src="{{URL::asset('public/admin/image/flags/'.$language->image)}}" title="{{$language->name}}"></span>
