...
<div class="panel-body">
          {!!Form::open(array('route'=>'admin.tour.store','id'=>'form-category','files'=>true,'class'=>'form-horizontal'))!!}
	        ...
	        <div class="tab-pane fade" id="tab-data">
	            <div class="content">
				    ...
				    <div class="form-group" id="category">
				        ...
				        <div class="col-sm-8">
                      		{!!Form::select('category', $categories ,'',array( "id"=>"input-category", "class"=>"form-control"))!!}
		            		<div id="tour-category" class="well well-sm" style="height: 150px; overflow: auto;">
					        	...
					        	@foreach($categories as $id => $name)
						        	@foreach(Input::old('category_id') as $cate_id)
						        		@if($cate_id==$id)
				    ...
				    <div class="form-group required">
                        ...
                        <div class="col-sm-2">
                        	@if(Input::old())
                            <input type="number" name="adult" value="{{Input::old('adult')}}" placeholder="Adult" id="input-adult" class="form-control">
                            @else
                            <input type="number" name="adult" value="1" placeholder="Adult" id="input-adult" class="form-control">
                            @endif
                        </div>
				    ...
				    <div class="form-group required">
                        ...
                        <div class="col-sm-3">
                            <div class="input-group date">
                            	@if(Input::old())
                                <input type="date" name="departure_time" value="{{Input::old('departure_time')}}" min="{{date('Y-m-d')}}" placeholder="Departure Time" data-date-format="YYYY-MM-DD" id="input-departure_time" class="form-control">
                                @else
                                <input type="date" name="departure_time" value="{{date('Y-m-d')}}" min="{{date('Y-m-d')}}" placeholder="Departure Time" data-date-format="YYYY-MM-DD" id="input-departure_time" class="form-control">
                                @endif
                        	</div>
                        </div>
                    </div>
				    ...
				    <div class="form-group required">
				        ...
				        <div class="col-sm-3">
				            <div class="input-group date">
				                ...
				                @else
				                <input type="date" name="date_available" value="{{date('Y-m-d')}}" placeholder="Date Available"id="input-date-available" class="form-control">
				                @endif	
				            </div>
				        </div>
				    </div>
								...
								<tr class ="schedule_row_{{$u}}">
									...
									<td class="col-sm-10">
										<ul class="nav nav-tabs" id="language_schedule_{{$u}}"> 
											@foreach($languages as $language)
											<li><a href="#schedule-{{$u}}{{$language->code}}" data-toggle="tab" aria-expanded="true"><img src="http://localhost/laravel/lar/public/admin/image/flags/{{$language->image}}" title="{{$language->name}}"> {{$language->name}} </a></li>
											@endforeach
										</ul>
									</td>
									<td></td>
								...
								<tr class ="attention_row_{{$o}}">
									<td class="col-sm-11">
										<ul class="nav nav-tabs" id="language_attention_{{$o}}"> 
											@foreach($languages as $language)
											<li><a href="#attention-{{$o}}{{$language->code}}" data-toggle="tab" aria-expanded="true"><img src="http://localhost/laravel/lar/public/admin/image/flags/{{$language->image}}" title="{{$language->name}}"> {{$language->name}} </a></li>
											@endforeach
										</ul>
									</td>
