...
$(document).ready(function () {
    ...
    $("#check_in")
            .change(function () {
                var str = "";
                var checkin = $("#check_in input").val();
                var n = $("#night select").val();
                var datein = new Date(checkin);
                var out = new Date(datein.getFullYear(), datein.getMonth(), datein.getDate() + Number(n) - 1);
                var dateout = out.toDateString();
                $('#check_out input').val(dateout);
            })
            .change();
    $("#check_out")
            .change(function () {
                var str = "";
                var checkin = $("#check_in input").val();
                var checkout = $("#check_out input").val();
                var datein = new Date(checkin);
                var dateout = new Date(checkout);
                var offset = dateout.getTime() - datein.getTime();
                var totalDays = Math.round((offset / 1000 / 60 / 60 / 24) + 1);
                $("#night select").val(totalDays);
            })
            .change();
    $("#night")
            .change(function () {
                ...
                var out = new Date(date.getFullYear(), date.getMonth(), date.getDate() + Number(n) - 1);
                var dateout = out.toDateString();
                $('#check_out input').val(dateout);
            })
            .change();
    ...
    $("#guest")
            .change(function () {
                ...
                if (n != "") {
                    ...
                    $("#hide select[name=\'children\']").val(0);
                }
            })
            .change();
